...
from src.post_install_linux.backend.env import TEMP_DIR, DISTRO
...
class Install_packages():
    ...
    def snaps_install(self):
        if DISTRO in ("debian", "fedora"):
            print("\033[1;34m===== 🔥 Installing Snaps =====\033[0m")

            if DISTRO == "fedora" and not os.path.exists("/snap"):
                utils.run_cmd(["ln", "-s", "/var/lib/snapd/snap", "/snap"], check=False, sudo=True, password=self.PASSWORD)
    ...
    def install_flatpaks(self):
        ...
        utils.run_cmd(["flatpak", "remote-add", "--if-not-exists"], check=True, sudo=True, password=self.PASSWORD)
        utils.run_cmd(["flathub", "https://dl.flathub.org/repo/flathub.flatpakrepo"], check=True, sudo=True, password=self.PASSWORD)
    ...
    def install_docker_fedora(self):
        ...
        docker_repo = utils.get_configs("docker")
        repo_path = "/etc/yum.repos.d/docker-ce.repo"
        utils.run_cmd(["tee", repo_path],sudo=True, password=self.PASSWORD, input=docker_repo.encode(), check=True)
        ...
        utils.run_cmd(["usermod", "-aG", "docker", os.getlogin()], sudo=True, password=self.PASSWORD,check=False)

        print("🚀 Enabling and starting Docker...")
        utils.run_cmd(["systemctl", "enable", "--now", "docker"], sudo=True, password=self.PASSWORD,check=True)

        print("🧪 Testing Docker with hello-world...")
        utils.run_cmd(["docker", "run", "hello-world"], sudo=True, password=self.PASSWORD, check=False)
