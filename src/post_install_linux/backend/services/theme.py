...
from post_install_linux.backend.env import TEMP_DIR, DISTRO, PASSWORD
...
def install_gtk_theme():
    ...
    utils.run_cmd(["./tweaks.sh", "-F"], cwd=path, sudo=True, password=PASSWORD)
    ...
    if os.path.isfile(sequoia_path):
        ...
        utils.run_cmd(["./tweaks.sh", "-g", "-nb", "-b", "sequoia.jpeg", "-t", "blue", "-c", "dark"], cwd=path, sudo=True, password=PASSWORD)
    else:
        utils.run_cmd(["./tweaks.sh", "-g", "-nb", "-t", "blue", "-c", "dark"], cwd=path, sudo=True, password=PASSWORD)

    utils.run_cmd(["flatpak", "override", "--filesystem=xdg-config/gtk-3.0"], sudo=True, password=PASSWORD)
    utils.run_cmd(["flatpak", "override", "--filesystem=xdg-config/gtk-4.0"], sudo=True, password=PASSWORD)
...
def install_themes_icons():
    ...
    if os.path.isdir(path):
        ...
        utils.run_cmd(["./install.sh", "-t", "all"], cwd=path, sudo=True, password=PASSWORD)
