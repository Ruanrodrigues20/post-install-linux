...
from src.utils.utils import get_data, check_connection
...
def setup_yay():
    ...
    yay_path = os.path.join(RESOURCES_DIR, "yay")
...
def set_configs_fastfetch():
    fastzip = Path("resources/fast.zip")
...
"""
    ...
    print("Aliases adicionados a ~/.bash_aliases")

    # Check if source line is in .bashrc, add if missing
    with open(bashrc, "r") as f:
        bashrc_content = f.read()

    if "source ~/.bash_aliases" not in bashrc_content:
        with open(bashrc, "a") as f:
            f.write("\nsource ~/.bash_aliases\n")
        print("Linha 'source ~/.bash_aliases' adicionada ao ~/.bashrc")
